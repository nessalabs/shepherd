...
| shutdown | Sets shutdown intent immediately and serializes cleanup. New scopes and spawns are rejected. Successful shutdown joins the sampler; cancellation retains its join for retry. Failure/cancellation can be retried; no early flag turns failure into success. |
