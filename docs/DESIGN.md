...
**observation and mechanism** ("PID 1432 is using 2 GB RSS"; "terminate this scope").
...
`ScopeClosed`, `ProcessReaped`. Raised (returned) by the aggregate; dispatched **in-process**
to same-context handlers; emitted at most once logically. See §3.4 for domain-vs-integration
events and handler design.
- **Integration Events** — the externally-meaningful subset (e.g. `ProcessTerminated`)
...
### 3.4 Events: domain vs integration (and how handlers work)

We deliberately separate **two kinds of events by type and mechanism**, so we never use one
where the other is required (using a domain event where an out-of-context integration event
is actually needed is the classic mistake). Shepherd is a single bounded context with no
database, so the "single transaction" that domain events participate in is the **state
transition committed under the registry lock**.

**1. Domain events — in-process, same bounded context, participate in correctness.**
- Used to react to a fact *within* Shepherd — e.g. modify/advance another part of the same
  context as part of completing one logical operation.
- **Pure production:** aggregate methods *return* `Vec<DomainEvent>`; the aggregate never
  dispatches (keeps the domain crate pure).
- **In-process dispatch:** an `EventDispatcher` (a mediator — a MediatR-style in-process
  dispatcher is perfectly adequate here; no message bus needed) routes events, **after the
...
- **Fail-loud & bounded:** deterministic handler ordering; a handler error is a typed
  `HandlerError` + `tracing`, never swallowed (no detached tasks whose failures vanish). A
  failed reap maps to `CleanupUnverified(ReapFailed)`, not a false success.
- Each handler **self-filters** (handles the kinds it cares about) — a registry of focused
  handlers instead of one growing `match`.
- "At most once logically" falls out of idempotent state transitions.

**2. Integration events — cross the boundary, out of context, decoupled.**
- Used to notify a **different** bounded context — the consuming application's policy layer
  (e.g. Nessa) — which lives outside Shepherd.
- Separate `IntegrationEvent` type published through an **outbound** `IntegrationEventPublisher`
  port (opt-in). Implemented as a **bounded, lossy-tolerant** channel (bounded broadcast with
  a lag/drop count) or a caller-supplied publisher the caller bridges to its own bus.
- **Hard rule:** an absent/slow/failing external consumer can **never** stall termination or
  affect Shepherd's internal state or invariants. This is exactly the separation that keeps
  domain and integration concerns from bleeding together.
- An app-layer **translator** maps the externally-meaningful *subset* of domain events into
  integration events — not every domain event becomes an integration event.

This also resolves the earlier subscription question: internal domain events + handlers are
**required** (in-process mediator, correctness); external observation is the **optional**
`IntegrationEventPublisher` (bounded, decoupled), clearly typed as integration events.
...
`ReapError`, `ShutdownError`, `HandlerError` (surfaced from event handlers, never swallowed),
plus domain errors (`ScopeClosed`, `UnknownProcess`, `UnknownScope`). Each carries enough context to branch on. Failure injection points (§13)
