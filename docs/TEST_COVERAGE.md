...
| Blocking / sync entry | NullBackend contract: natural/forced exit, scope isolation, with_scope result/panic/nesting/partial spawn, `run` completion and deadline kill, shared-supervisor threads, owned current-thread runtime, current-thread `from_runtime` `with_scope` spawn/wait (body must not sit inside `block_on`), current-thread panic + `wait_scope_cleanup`, BYO multi-thread handle, both Tokio flavors, current-thread `from_handle` refusal (no deadlock), deadline-0 and 1ms leftover cleanup, spawn errors that cannot hide terminate failure, 32-cycle verified loop, slow-spawn admission then reap, clone-after-drop, run-after-shutdown, and `into_verified`. Real processes: exit codes, graceful/force, isolation, cleared env, captured and capped output, deadline covering wait (Unix close-stdio-then-hang, cleared-env sleeper without PATH), missing program, four concurrent deadline runs, group descendant sweep, nested real `with_scope`, current-thread `with_scope` wait, repeated runs, and `run` from inside a multi-thread application runtime. |
