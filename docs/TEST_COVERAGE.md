...
| Blocking / sync entry | NullBackend contract: natural/forced exit, scope isolation, with_scope result/panic/nesting, `run` completion and deadline kill, shared-supervisor threads, owned current-thread runtime, BYO multi-thread handle, both Tokio flavors, current-thread `from_handle` refusal (no deadlock), deadline-0 and 1ms leftover cleanup, and spawn errors that cannot hide a later terminate failure. Real processes on Linux/macOS/Windows: exit codes, graceful/force, isolation, cleared env, captured and capped output, deadline covering wait (Unix close-stdio-then-hang, Windows hanging cmd), group descendant sweep, nested real `with_scope`, and `run` from inside a multi-thread application runtime. |
