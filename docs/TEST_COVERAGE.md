...
| Blocking / sync entry | NullBackend contract: natural/forced exit, scope isolation, with_scope result/panic/nesting, `run` completion and deadline kill, shared-supervisor threads, owned current-thread runtime, BYO multi-thread handle, and both Tokio flavors calling into an owned or same-runtime supervisor. Real processes on Linux/macOS/Windows: exit codes, graceful/force, isolation, cleared env, captured and capped output, deadline covering wait (including Unix close-stdio-then-hang), group descendant sweep, nested real `with_scope`, and `run` from inside a multi-thread application runtime. |
